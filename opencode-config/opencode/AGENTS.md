...
- **Triage "config not working" reports from the runtime outward, not the source inward.** When a user reports a config change (model, provider, permission, MCP) "isn't working": (1) Check for a stale long-running process first — compare the consuming process's start time (`ps -o lstart -p <pid>`) with the config file's mtime (`stat -c %y <file>`). A daemon that started before the edit is serving the old config by definition (e.g. `opencode serve` loads config once at startup, never hot-reloads). (2) Use the tool's own debug output from a FRESH process (`opencode debug config`) — if it differs from live behavior, the running process is stale; restart it, don't debug the source. (3) Verify the EXACT identifier the user selected matches the configured key — variants, aliases, and dated checkpoints (`-0731`, `~latest`) are separate keys, and a pin on the base ID does not cover them. (4) Check persistent state files (recent lists, saved selections, localStorage) that can silently override config even when the config file is correct. Only after these runtime checks should you read source code, and then only to confirm the mechanism works — not to explain the symptom.
...
Use Gortex MCP tools for code understanding:
- `gortex_read` operation:`"symbols"` – High-level symbol overview of a file
- `gortex_search` operation:`"symbols"` – Find classes, methods, functions by name pattern
- `gortex_relations` operation:`"references"` – Find references to a symbol
- `gortex_search` operation:`"text"` – Search text/regex patterns in the project (fallback: `rg` > `grep`)
- `gortex_search` operation:`"files"` – Find files by name
- `gortex_read` operation:`"file"` – Read a file

### File & Directory Operations
Use built-in tools:
- `write` – Create new/overwrite existing files
- `edit` – Precise text replacements in a file
...
### Memory (Gortex)
- `gortex_recall` operation:`"memories"` – List or search persistent memories
- `gortex_recall` operation:`"notes"` – Read specific notes/memories
- `gortex_remember` operation:`"memory"` – Write a new memory
...
- `gortex_workspace` operation:`"project"` – Set active project scope
- `gortex_workspace` operation:`"info"` – Inspect current workspace config

### Symbol Editing
Use gortex when modifying code definitions:
- `gortex_edit` operation:`"symbol"` – Replace a symbol's full definition
- `gortex_edit` operation:`"symbol"` with appropriate `match` – Insert content before/after a symbol
- `gortex_refactor` operation:`"rename"` – Rename a symbol across the project
- `gortex_refactor` operation:`"delete"` – Delete a symbol safely
...
- Always check for the appropriate Gortex tool before falling back to generic system tools
